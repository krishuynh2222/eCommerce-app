...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      let orderItems = []; //hold the processed order items (productinfo, size, quantity)

      for (const items in cartItems) {
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            //products.find() returns a valid product. 
      ...
      //Choose the difference paymentMethod
      switch (method) {
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            ...
            <div onClick={() => setMethod('paypal')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              ...
              <img className="h-10 w-20 mx-2" src={assets.paypal_logo} alt="" />
            </div>
